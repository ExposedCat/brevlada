...
class EmailStorage:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_messages_folder_account ON messages(folder, account_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_messages_date ON messages(date_sent)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_messages_thread ON messages(thread_subject, folder, account_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_messages_flags ON messages(is_read, is_flagged)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_messages_search ON messages(subject, sender_name, sender_email)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_threads_folder_account ON threads(folder, account_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_threads_date ON threads(latest_date)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_attachments_message ON attachments(message_uid)")
    ...
    def store_messages(self, messages: List, folder: str, account_id: str):
        ...
        logging.debug(f"EmailStorage: Storing {len(messages)} messages for folder '{folder}', account_id '{account_id}'")
        with self._lock:
        ...
        try:
            with self.get_connection() as conn:
                ...
                """, (
                    ...
                    json.dumps(recipients), json.dumps(cc), json.dumps(bcc),
                    json.dumps(reply_to), date_sent, json.dumps(flags),
                    is_read, is_flagged, is_deleted, is_draft, is_answered,
                    has_attachments, body_text, body_html, json.dumps(headers),
                    json.dumps(envelope), json.dumps(bodystructure),
                    thread_subject, json.dumps(thread_references),
                    in_reply_to, references, 'synced', datetime.now()
                ))
                ...
                if uid is not None:
                    for attachment in attachments:
                        if isinstance(attachment, dict):
                            self.store_attachment_dict(attachment, uid, folder, account_id)
                        else:
    ...
    def store_attachment(self, attachment, message_uid: int, folder: str, account_id: str):
        ...
        with self.get_connection() as conn:
            ...
            """, (
                message_uid, folder, account_id, attachment.filename,
                attachment.content_type, attachment.size, attachment.part_id,
                attachment.is_inline, attachment.content_id
            ))

    def store_attachment_dict(self, attachment_dict, message_uid: int, folder: str, account_id: str):
        ...
        with self.get_connection() as conn:
            ...
            """, (
                message_uid, folder, account_id, attachment_dict.get('filename', ''),
                attachment_dict.get('content_type', ''), attachment_dict.get('size', 0),
                attachment_dict.get('part_id', ''), attachment_dict.get('is_inline', False),
                attachment_dict.get('content_id', '')
            ))

    def get_messages(self, folder: str, account_id: Optional[str] = None, limit: int = 50, offset: int = 0) -> List[Dict]:
        """Get messages from storage"""
        logging.debug(f"EmailStorage: Getting messages for folder='{folder}', account_id='{account_id}', limit={limit}, offset={offset}")
        ...
        try:
            with self.get_connection() as conn:
                logging.debug(f"EmailStorage: Executing query for messages in folder '{folder}'")
                cursor = conn.execute("""
                ...
                for i, row in enumerate(rows):
                    ...
                    # Get attachments
                    message_data['attachments'] = self.get_message_attachments(row['uid'], folder, account_id)
                    messages.append(message_data)

                logging.info(f"EmailStorage: Successfully retrieved {len(messages)} messages for folder '{folder}'")
                return messages
        except Exception as e:
            logging.error(f"EmailStorage: Error getting messages for folder '{folder}': {e}")
            logging.debug(f"EmailStorage: Exception details: {type(e).__name__}: {str(e)}")
            raise
    ...
    def get_message_attachments(self, message_uid: int, folder: str, account_id: str) -> List[Dict]:
        """Get attachments for a specific message"""
        logging.debug(f"EmailStorage: Getting attachments for message uid={message_uid}")
        try:
            with self.get_connection() as conn:
                cursor = conn.execute("""
                ...
                logging.debug(f"EmailStorage: Found {len(attachments)} attachments for message uid={message_uid}")
                return attachments
        except Exception as e:
            logging.error(f"EmailStorage: Error getting attachments for message uid={message_uid}: {e}")
            return []

    def search_messages(self, query: str, folder: str, account_id: str, limit: int = 50) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            ...
            cursor = conn.execute("""
                ...
                ORDER BY date_sent DESC
                LIMIT ?
            """, (folder, account_id, search_query, search_query, search_query, search_query, limit))
    ...
    def update_sync_status(self, account_id: str, folder: str, status: str, last_uid: Optional[int] = None):
