...
class IMAPConnection:
    ...
    def connect(self) -> bool:
        ...
        with self.lock:
            try:
                ...
                logging.info(f"Connecting to IMAP server: {server}:{port} for {self.email}")
    ...
    def _get_oauth2_token(self) -> Optional[str]:
        ...
        try:
            import dbus
            bus = dbus.SessionBus()
            account_obj = bus.get_object("org.gnome.OnlineAccounts", self.account_data["path"])
                ...
                try:
                    if not self.connection or not self.is_authenticated:
                        if not self.connect() or not self.authenticate():
                            raise Exception("Failed to establish authenticated connection")
                    ...
                    logging.warning(f"IMAP connection error for {self.email} (attempt {attempt + 1}): {e}")
                    ...
                    logging.error(f"Unexpected error executing IMAP command for {self.email}: {e}")
...
class IMAPConnectionManager:
    ...
    def get_connection(self, account_data: Dict[str, Any], mail_settings: Dict[str, Any]) -> IMAPConnection:
                    ...
                    with self.lock:
                        ...
                        for email, connection in self.connections.items():
                            if connection.is_idle(timeout_seconds=600):  # 10 minutes
                                logging.debug(f"Cleaning up idle connection for {email}")
                                connection.logout()
